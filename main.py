...
# GitHub API client
class GitHubClient:
    def __init__(self, token: str, enterprise_url: str):
        self.token = token
    ...
    async def get_consumed_licenses(self) -> Dict:
        """Get consumed licenses information from the GitHub Enterprise API."""
        session = await self.ensure_session()
        
        logger.info(f"Making API request to: {self.enterprise_url}")
        
        try:
            async with session.get(self.enterprise_url) as response:
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[None]:
    ...
    github_client = GitHubClient(token=token, enterprise_url=enterprise_url)
...
async def list_consumed_licenses(ctx: Context, include_users: bool = False) -> ConsumedLicensesResponse:
    ...
    consumed_licenses_data = await github_client.get_consumed_licenses()
...
async def get_github_consumed_licenses(dummy: str) -> ConsumedLicensesResponse:
    ...
    try:
        async with session.get(enterprise_url) as response:
            if response.status == 200:
...
if __name__ == "__main__":
    ...
    enterprise_url = os.environ.get("GITHUB_ENTERPRISE_URL")
    if enterprise_url:
        logger.info(f"GitHub Enterprise URL: {enterprise_url}")
    else:
